...
shaka.media.MediaSourceEngine = class {
   ...
   * @return {!Promise}
   */
  async resyncForSequenceMode(contentType, timestampOffset) {
    const ContentType = shaka.util.ManifestParserUtils.ContentType;
  ...
  /**
   * Adjust timestamp offset to maintain AV sync across discontinuities.
   * Sets timestamp offset to end of current buffered region, so that next
   * segment gets appended immediately after even if its internal timestamp
   * begins at 0.
   * Only used in segments mode.
   *
   * @param {shaka.util.ManifestParserUtils.ContentType} contentType
   * @return {!Promise}
   */
  async resyncForSegmentsMode(contentType) {
    const ContentType = shaka.util.ManifestParserUtils.ContentType;

    goog.asserts.assert(!this.sequenceMode_,
        'resync only used with segments mode!');
    ...
    const newTimestampOffset = Number(this.bufferEnd(contentType));
    await this.enqueueOperation_(
        contentType,
        () =>
          this.setTimestampOffset_(contentType, newTimestampOffset));
  }
